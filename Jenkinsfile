  ...
  agent {label 'main'}
    stages{
       stage('hosting application'){
        steps{
          sh "ls"
          sh "aws rds create-db-instance --db-instance-identifier test-mysql-instance --db-name cpms --db-instance-class db.t2.micro --vpc-security-group-ids  sg-04638412bef8421f5 --engine mysql --engine-version 5.7 --db-parameter-group-name default.mysql5.7 --publicly-accessible  --master-username admin --master-user-password shiva123 --allocated-storage 10 --region us-east-2"
          sleep(450)
          script{
          ...
          script{
              def cmd = "aws elbv2 create-target-group --name my-targets --protocol HTTP --port 80 --target-type instance --vpc-id vpc-021ac10c8a336a064 --region us-east-2"
              def output = sh(script: cmd,returnStdout: true)
           ...
           sh "aws autoscaling create-launch-configuration --launch-configuration-name my-lc3-cli --image-id ami-0fb653ca2d3203ac1 --instance-type t2.micro --security-groups sg-04638412bef8421f5 --key-name shivakey --iam-instance-profile demorole --user-data file://userdata.txt --region us-east-2"
