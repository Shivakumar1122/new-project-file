  ...
  agent {label 'main'}
    stages{
       stage('hosting application'){
        steps{
          sh "ls"
          sh "aws rds create-db-instance --db-instance-identifier test-mysql-instance1 --db-name cpms --db-instance-class db.t2.micro --vpc-security-group-ids  sg-04638412bef8421f5 --engine mysql --engine-version 5.7 --db-parameter-group-name default.mysql5.7 --publicly-accessible  --master-username admin --master-user-password shiva123 --allocated-storage 10 --region us-east-2"
          sleep(450)
          script{
